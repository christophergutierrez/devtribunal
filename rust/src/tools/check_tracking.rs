...
const IDE_EXTENSIONS: &[(&str, &str)] = &[(".swp", "Vim swap file"), (".swo", "Vim swap file")];
...
fn classify_tracked_file(path: &str) -> Option<TrackingIssue> {
    ...
    for &(prefix, reason) in ARTIFACT_PREFIXES {
        if path.starts_with(prefix) || path.contains(&format!("/{prefix}")) {
            return Some(TrackingIssue {
                path: path.to_string(),
                category: TrackingCategory::BuildArtifact,
                reason,
            });
        }
    }
    ...
    for &(ext, reason) in BINARY_EXTENSIONS {
        if path.ends_with(ext) {
            return Some(TrackingIssue {
                path: path.to_string(),
                category: TrackingCategory::LargeBinary,
                reason,
            });
        }
    }
    ...
    for &(pattern, reason) in IDE_OS_FILES {
        if filename == pattern {
            return Some(TrackingIssue {
                path: path.to_string(),
                category: TrackingCategory::IdeOrOs,
                reason,
            });
        }
    }
...
fn generate_fixes(issues: &[TrackingIssue]) -> String {
    ...
    let secrets: Vec<_> = issues
        .iter()
        .filter(|i| i.category == TrackingCategory::Secret)
        .collect();
    let artifacts: Vec<_> = issues
        .iter()
        .filter(|i| i.category == TrackingCategory::BuildArtifact)
        .collect();
    let binaries: Vec<_> = issues
        .iter()
        .filter(|i| i.category == TrackingCategory::LargeBinary)
        .collect();
    let ide: Vec<_> = issues
        .iter()
        .filter(|i| i.category == TrackingCategory::IdeOrOs)
        .collect();
...
fn format_tracking_audit(result: &TrackingAuditResult) -> String {
    ...
    if !result.tracked_issues.is_empty() {
        lines.push(format!(
            "## Tracked Files That Shouldn't Be ({} issues)",
            result.tracked_issues.len()
        ));
        lines.push(String::new());

        for category in &[
            TrackingCategory::Secret,
            TrackingCategory::BuildArtifact,
            TrackingCategory::LargeBinary,
            TrackingCategory::IdeOrOs,
        ] {
    ...
    if !result.ignored_source.is_empty() {
        lines.push(format!(
            "## Ignored Source Files ({} files)",
            result.ignored_source.len()
        ));
        lines.push(String::new());
        lines.push(
            "These source files are matched by `.gitignore` patterns — verify this is intentional:"
                .to_string(),
        );
...
pub async fn execute_check_tracking(repo_path: &str) -> ToolResult {
    ...
    let tracked_issues: Vec<TrackingIssue> = result
        .stdout
        .lines()
    ...
    if ignored_source.len() > 100 {
        errors.push(format!(
            "Found {} ignored source files — showing first 50",
            ignored_source.len()
        ));
    }

    let ignored_source_capped: Vec<IgnoredSourceFile> =
        ignored_source.into_iter().take(50).collect();
...
mod tests {
    ...
    fn test_classify_secrets() {
        assert_eq!(
            classify_tracked_file(".env").unwrap().category,
            TrackingCategory::Secret
        );
        assert_eq!(
            classify_tracked_file("config/api.key").unwrap().category,
            TrackingCategory::Secret
        );
        assert_eq!(
            classify_tracked_file("certs/server.pem").unwrap().category,
            TrackingCategory::Secret
        );
        assert_eq!(
            classify_tracked_file("id_rsa").unwrap().category,
            TrackingCategory::Secret
        );
        assert_eq!(
            classify_tracked_file("path/with/secret/in/it")
                .unwrap()
                .category,
            TrackingCategory::Secret
        );
    ...
    fn test_classify_artifacts() {
        assert_eq!(
            classify_tracked_file("node_modules/lodash/index.js")
                .unwrap()
                .category,
            TrackingCategory::BuildArtifact
        );
        assert_eq!(
            classify_tracked_file("src/main.pyc").unwrap().category,
            TrackingCategory::BuildArtifact
        );
        assert_eq!(
            classify_tracked_file("target/debug/binary")
                .unwrap()
                .category,
            TrackingCategory::BuildArtifact
        );
    ...
    fn test_classify_binaries() {
        assert_eq!(
            classify_tracked_file("lib/native.so").unwrap().category,
            TrackingCategory::LargeBinary
        );
        assert_eq!(
            classify_tracked_file("assets/archive.zip")
                .unwrap()
                .category,
            TrackingCategory::LargeBinary
        );
    ...
    fn test_classify_ide() {
        assert_eq!(
            classify_tracked_file(".DS_Store").unwrap().category,
            TrackingCategory::IdeOrOs
        );
        assert_eq!(
            classify_tracked_file(".idea/workspace.xml")
                .unwrap()
                .category,
            TrackingCategory::IdeOrOs
        );
        assert_eq!(
            classify_tracked_file("src/.main.rs.swp").unwrap().category,
            TrackingCategory::IdeOrOs
        );
    }
    ...
    fn test_secret_component_match_not_substring() {
        ...
        assert_eq!(
            classify_tracked_file("config/secret/api.yml")
                .unwrap()
                .category,
            TrackingCategory::Secret
        );
        ...
        assert_eq!(
            classify_tracked_file("config/secrets/db.yml")
                .unwrap()
                .category,
            TrackingCategory::Secret
        );
