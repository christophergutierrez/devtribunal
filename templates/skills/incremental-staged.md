...
3. **Review files**: For each staged file, call the matching `review_*` MCP tool. Use these tool names:
...
4. **Check documentation**: If any README or documentation files are in the staged list, call `check_docs` on them.
...
7. **Present results**: Show the final action plan, noting this review covers only staged changes.
